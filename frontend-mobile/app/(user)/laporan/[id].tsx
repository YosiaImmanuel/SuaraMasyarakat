...
import {
  View, Text, StyleSheet, ScrollView, ActivityIndicator, TouchableOpacity, TextInput, Alert, Image, Modal, Dimensions,
} from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import { Ionicons } from "@expo/vector-icons";
import { useLocalSearchParams, useRouter } from "expo-router";
import { fetchLaporanById, fetchComments, createComment, deleteComment } from "@/lib/api";
...
export default function LaporanDetailScreen() {
  ...
  const handleComment = async () => {
    ...
    try {
      ...
      if (res.ok) { setNewComment(""); const updated = await fetchComments(id!); setComments(updated || []); }
    } catch {
  ...
  const handleDeleteComment = (commentId: number) => {
    Alert.alert("Hapus Komentar", "Yakin ingin menghapus komentar ini?", [
      ...
      { text: "Hapus", style: "destructive", onPress: async () => { try { const { res } = await deleteComment(commentId); if (res.ok) setComments((prev) => prev.filter((c) => c.id !== commentId)); } catch {} } },
    ]);
  };
...
const styles = StyleSheet.create({
  ...
  badge: { paddingHorizontal: 12, paddingVertical: 4, borderRadius: 8, marginBottom: 12 },
